...
{\tt v \tup{time} link-\tup{operation} \tup{node1} \tup{node2}}.
...
\item Two instance procedures in the base class ,
  ...
  \proc[interval, operation]{set-event} schedules {\tt operation}
  after {\tt interval} seconds from the current time; it uses the
  procedure \proc[]{set-event-exact} below.

  \proc[fireTime, operation]{set-event-exact} schedules {\tt operation}
...
\item
  \proc[]{Node::intf-changed} will notify any {\tt rtObject}
