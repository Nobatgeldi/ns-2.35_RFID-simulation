...
\proc[a g]{join-group} &
        An {\tt agent} at a node that joins a particular group invokes
        ``{\tt node join-group \<agent\> \<group\>}''.  The
        node signals the mrtObject to join the particular {\tt group},
        and adds {\tt agent} to its list of agents at that
        {\tt group}.  It then adds {\tt agent} to all replicators
        associated with {\tt group}. \\
\proc[a g]{leave-group} &
        {\tt Node~instproc~leave-group} reverses the process
        described earlier.  It disables the outgoing interfaces to the
        receiver agents for all the replicators of the group, deletes
        the receiver agents from the local {\tt Agents\\_} list; it
        then invokes the arbiter instance's
        \proc[]{leave-group}.\\[2ex]
\proc[s g iif oiflist]{add-mfc} &
        {\tt Node~instproc~add-mfc} adds a \textit{multicast forwarding cache}
        entry for a particular \tup{source, group, iif}.
        The mechanism is:
        \begin{itemize}
        \item create a new replicator (if one does not already exist),
        \item update the {\tt replicator\\_} instance variable array at the node,
...
\proc[s g oiflist]{del-mfc} &
        disables each oif in {\tt oiflist} from the replicator for \tup{s, g}.\\
\end{\par\tabular{\textwidth}{rX}}
...
{\tt CtrMcastComp} is the centralised multicast route computation agent.
\begin{\par\tabular{\textwidth}{rX}}
\proc[]{reset-mroutes} & 
...
{\tt wkgroups} &
        ...
        addresses, indexed by the group name.  {\tt wkgroups}(Allocd)
...
{\tt head\\_} & 
        ...
        object contains the instance variable, {\tt link\\_}, that
