...
This section describes the structure of unicast routing in \emph{ns}.
...
of routing in \emph{ns}.

The procedures and functions described in this chapter can be found in
\textasciitilde\emph{ns}/{tcl/lib/ns-route.tcl}, \textasciitilde\emph{ns}/{tcl/rtglib/route-proto.tcl}, 
\textasciitilde\emph{ns}/{tcl/mcast/McastProto.tcl}, and \textasciitilde\emph{ns}/{rtProtoDV.\{cc, h\}}.
...
A routing strategy is a general mechanism by which \emph{ns}\
will compute routes for the simulation.
There are four routing strategies in \emph{ns}:
...
In \emph{ns}, we blur the distinction between strategy and protocol for
...
then \emph{ns}\ will run Static routing on all the nodes in the topology.
...
Each protocol agent stores an array of route preferences, {\tt rtpref\_}.
...
a class variable, {\tt preference\_}, for that protocol.
The current defaults are:
\begin{verbatim}
        Agent/rtProto set preference\_ 200               # global default preference;
        Agent/rtProto/Direct\footnote{Direct is a special routing strategy that is used in conjunction with Dynamic routing.  We will describe this in greater detail as part of the route architecture description.} set preference\_ 100
        Agent/rtProto/DV set preference\_ 120
\end{verbatim}
...
is invoked as {\tt \$ns cost \tup{node1} \tup{node2} \tup{cost}},%$
...
The instance variable {\tt multiPath\_} determines whether or not
...
\begin{verbatim}
        Node set multiPath\_ 1 # All new nodes in the simulation use multiPaths where applicable;
{\rm or alternately}
        set n1 [\$ns Node] # only enable \$n1 to use multiPaths where applicable;
        \$n1 set multiPath\_ 1
\end{verbatim}%\$
...
the default route computation mechanism  in \emph{ns}.
...
Distributed Bellman-Ford (or Distance Vector) routing in \emph{ns}.
The implementation sends periodic route updates every {\tt advertInterval}.
...
Each DV agent uses a default {\tt preference\_} of 120.
The value is determined by the class variable of the same name.

Each agent uses the class variable {\tt INFINITY} (set at 32)
...
For a more complete example, see {\tt tcl/ex/many\_tcp.tcl}.
...
and constructs an instance variable, {\tt rtprotos\_}, as an array;
...
reads the {\tt rtprotos\_} instance variable, and 
...
For each protocol \tup{rt-proto} indexed in the {\tt rtprotos\_} array,
this routine invokes
{\tt Agent/rtProto/\tup{rt-proto} init-all rtprotos\_(\tup{rt-proto})}.

If there are no elements in {\tt rtprotos\_},
the routine invokes Static routing, as
{\tt Agent/rtProto/Static init-all}.
...
The procedure queries the appropriate {\tt rtObject} entities
if they exist
...
It subsequently invokes its {\tt compute-routes}.
...
in its instance variable {\tt node\_}, and from the node to itself,
through the Node instance procedure
\proc[]{init-routing} and the Node instance variable {\tt rtObject\_}.
It then initializes an array of
{\tt nextHop\_}, {\tt rtpref\_}, {\tt metric\_}, {\tt rtVia\_}.
The index of each of these arrays is the handle of the destination node.

The {\tt nextHop\_} contains the link that will be used to reach the
particular destination;
{\tt rtpref\_} and {\tt metric\_} are
the preference and metric for the route installed in the node;
{\tt rtVia\_} is the name of the agent whose route is installed in the node.
...
in its array of protocols, {\tt rtProtos\_}.
...
The class maintains the instance variable {\tt addr\_}, and the
instance variable arrays, {\tt metric\_} and {\tt rtpref\_};
...
variable array, {\tt ifs\_};
the corresponding status instance variable array is {\tt ifstat\_}.
...
is invoked by the {\tt rtObject} at the node.
It stores a pointer to the rtObject, in its instance variable
{\tt rtObject\_}, for later manipulation or retrieval.
...
the instance variable array {\tt routes\_} stores a count of the number of
...
\item   % class Link
  ...
  The instance variable {\tt cost\_}
...
variable array, {\tt elements\_}, and then invokes the instproc-like.
...
however, no such code is currently implemented in \emph{ns}.
...
This dynamicDM implementation in \emph{ns}\ assumes neighbor nodes
...
{\tt rtObject\_ flag-multicast 1} whenever it receives a route update as well,
...
of {\tt nextHop\_}, {\tt rtpref\_}, and {\tt metric\_}, indexed by 
...
and creates the relevant {\tt rtPeer} objects.
...
after {\tt advertInterval} jittered slightly to avoid
...
{\tt msg\_} indexed by a non-decreasing integer as index.
The instproc-like only sends the index to {\tt msg\_} to the remote peer.
...
routing in \emph{ns}. Routelogic, essentially, represents the routing table
...
scripts:
\begin{flushleft}
{\tt \$ns\_ rtproto \<routing-proto\> \<args\>}%$
...
{\tt \$ns\_ compute-routes}%$

This command computes {\tt next\_hop} information for all nodes in the
topology using the topology connectivity. This {\tt next\_hop} info is
...
{\tt \$ns\_ dump-routelogic-nh}%$

This dumps next hop information in the routing table.

{\tt \$ns\_ dump-routelogic-distance}%$
