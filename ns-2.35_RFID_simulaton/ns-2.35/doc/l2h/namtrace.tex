...
\begin{verbatim}
        if (namChan\_ != 0)
                sprintf(nwrk\_,
                        "%c -t "TIME\_FORMAT" -s %d -d %d -p %s -e %d -c %d
...
The first section of a trace file must contain initialization information. All initialization events will have the flag {\tt -t *}.  This tells nam that this event needs to be parsed before any animation has started. 
\begin{description}
\item[Version]
...
The flag {\tt -v 1.0a5} tells nam that this script requires a version of nam > 1.0a5.  For more information on this event look at the file tcl/stats.tcl under the procedure nam\_analysis.
...
\item[Hierarchy] Hierarchical address information is defined by:
  ...
  {\tt A -t \<time\> -n \<levels\> -o \<address-space size\> -c \<mcastshift\> -a \<mcastmask\> -h \<nth level\> -m \<mask in nth level\> -s \<shift in nth level\>}\\
...
{\tt c -t \<time\> -i \<color id\> -n \<color name\>}\\
...
{\tt n -t \<time\> -a \<src-addr\> -s \<src-id\> -S \<state\> -v \<shape\> -c \<color\> -i \<l-color\> -o \<color\>}\\
...
{\tt n -t * -a 4 -s 4 -S UP -v circle -c tan -i tan}\\
...
{\tt l -t \<time\> -s \<src\> -d \<dst\> -S \<state\> -c \<color\> -o orientation -r \<bw\> -D \<delay\>}\\
...
{\tt \<type\> -t \<time\> -e \<extent\> -s \<source id\> -d \<destination id\> -c \<conv\> -i \<id\>}\\
...
For example {\tt  -x \{0.1 -2147483648.0 -1 ------- SRM\_SESS\} } denotes an
...
They are created by:\\
{\tt m -t \<time\> -n \<mark name\> -s \<node\> -c \<color\> -h \<shape\> -o \<color\>}\\
and can be deleted by:\\
{\tt m -t \<time\> -n \<mark name\> -s \<node\> -X}\\
...
{\tt a -t \<time\> -n \<agent name\> -s \<src\>}\\
Because in \emph{ns}, agents may be detached from nodes, an agent may be 
deleted in nam with:\\
{\tt a -t \<time\> -n \<agent name\> -s \<src\> -X}\\
...
{\tt f -t \<time\> -s \<src\> -a \<agentname\> -T \<type\> -n \<varname\> -v \<value\> -o \<prev value\>}\\
Flag {\tt <type>} is 
\begin{description}
...
However, only {\tt v} is implemented in \emph{ns}. 
...
{\tt f -t 0.00000000000000000 -s 1 -n C1\_ -a srm(1) -v 2.25000 -T v}\\

Features may be deleted using:\\
{\tt f -t \<time\> -a \<agent name\> -n \<var name\> -o \<prev value\> -X}\\
...
{\tt v -t \<time\> -e \<nam tcl procedure call\>}\\
...
Normally the user chooses a playback rate via the rate slider in the animation window.  A trace file may set the playback rate via the {\tt set\_rate\_ext} command:

{\tt v -t \<time\> -e set\_rate\_ext \<time\> \<update-peers?\>}

For example:\\
{\tt v -t 2.5 -e set\_rate\_ext 20ms 1}

For compatibility with earlier versions of nam, the {\tt set\_rate} command is also supported.  Instead of specifying the step size directly, you use $10\times\log_{10} \textit{<time-in-seconds>}$.  For example:

{\tt v -t 2.5 -16.9897000433602 1}

In order to have more readable files, {\tt set\_rate\_ext} is preferred.

\subsubsection{Annotation}
This procedure is used for displaying text annotation at specfic times:

{\tt v -t \<time\> -e sim\_annotation \<time\> \<unique sequence id\> \<text to display\>}\\

For example:\\
{\tt v -t 4 -e sim\_annotation 4 3 node 0 added one mark}\\

This line calls a special tcl function {\tt sim\_annotation} in
nam, which inserts the given string {\tt node 0 added one mark}
into nam's annotation pane.  Look at  
{\tt Animator instproc sim\_annotation} in 
...
{\tt v -t 1.0 -e node\_tclscript \<node\_id\> \<button label\> \<tcl script\>}\\

This line when read in a tracefile will add an extra button to node objects that
will execute a tcl script when clicked.

For example:\\
{\tt v -t 1.0 -e node\_tclscript 2 "Echo Hello" \{puts [exec echo hello]\}}\\
...
above may be found in the following files: \emph{ns}/trace.cc, 
\emph{ns}/trace.h, \emph{ns}/tcl/lib/ns-namsupp.tcl.
...
This is a listing of all possible nam trace event codes and the flags associated with them.  It was taken from the source code in the file parser.cc.  You can generate your own table by running {\tt nam -p}.
...
This section describes different APIs in \emph{ns} that may be used to
...
\emph{ns}/tcl/lib/ns-namsupp.tcl.
Demonstration of nam APIs may be found in \emph{ns}/tcl/ex/nam-example.tcl.
...
{\tt \$ns duplex-link-op \<attribute\> \<value\>} %$
...
{\tt tracevar} method of the \emph{ns} agent can be used to create feature
...
{\tt \$ns trace-annotate \<annotation\>} %$
