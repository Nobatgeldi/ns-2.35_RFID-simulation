...
do
	...
	do
		...
		linhas_trafego=$(echo "scale=10; $(awk -f processar_dr1.awk cenario2.$j.filtrado.$i.$5.csv)"|bc -l)
		...
		linhas_trafego=$(echo "scale=10; ($linhas_trafego+$(awk -f processar_dr1.awk cenario2.$j.filtrado.$i.$5.csv))"|bc -l)
